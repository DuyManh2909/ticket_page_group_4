...
const DiscoveryThemePage: React.FC<DiscoveryThemePageProps> = ({
  theme,
  onSelectEvent,
}) => {
  const filteredEvents = MOCK_EVENTS.filter(
    (e) => e.category === theme.category,
  );
    ...
    <div className="pb-24 animate-[fadeIn_0.5s_ease-out]">
      ...
      <section className="relative h-[85vh] flex items-center justify-center overflow-hidden mb-32">
        <div className="absolute inset-0">
          ...
          <div
            className={`absolute inset-0 bg-gradient-to-b from-slate-950 via-transparent to-slate-950 opacity-90`}
          />
        </div>

        <div className="relative z-10 text-center px-6 max-w-5xl">
          <p
            className={`font-black uppercase tracking-[0.5em] text-xs mb-8 ${theme.colors.primary} animate-pulse-neon`}
          >
            {theme.subtitle}
          </p>
      ...
      <section className="max-w-7xl mx-auto px-6 mb-40">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-20 gap-8">
          <div>
            <p className="text-slate-600 font-black uppercase tracking-[0.4em] text-[10px] mb-3">
      ...
      <section className="max-w-7xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-3 gap-16 mb-40">
        <div className="lg:col-span-2 bg-slate-950/50 rounded-[4rem] p-16 border border-white/5 flex flex-col md:flex-row gap-12 hover:border-purple-500/10 transition-all shadow-2xl">
          <div className="w-full md:w-64 h-64 flex-shrink-0 relative group">
            <img
              src={theme.playlistImg}
              className="w-full h-full object-cover rounded-[2rem] shadow-2xl group-hover:scale-105 transition-transform duration-700 grayscale-[40%] group-hover:grayscale-0"
            />
            <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity bg-black/60 rounded-[2rem]">
              <div className="w-16 h-16 bg-white rounded-full flex items-center justify-center text-black font-black text-xl shadow-2xl">
                ▶
              </div>
            </div>
          </div>
          <div className="flex-grow flex flex-col justify-center">
            <h3
              className={`text-3xl font-black uppercase italic mb-6 ${theme.colors.primary}`}
            >
              Giai điệu tâm hồn
            </h3>
            <p className="text-xl text-slate-400 mb-10 leading-relaxed font-light italic">
              Hãy để âm nhạc chuẩn bị cho trái tim bạn trước khi bước vào không
              gian của {theme.title.toLowerCase()}.
            </p>
            <div className="flex flex-wrap gap-3">
              {["Soul", "Atmosphere", "Energy", "Deep"].map((tag) => (
                <span
                  key={tag}
                  className="px-5 py-2 bg-white/5 rounded-full text-[9px] font-black uppercase tracking-widest text-slate-400 border border-white/5"
                >
                  #{tag}
                </span>
              ))}
        ...
        <div
          className={`p-16 bg-gradient-to-br ${theme.colors.gradient} rounded-[4rem] text-white shadow-2xl flex flex-col justify-center`}
        >
          <p className="text-[10px] font-black uppercase tracking-[0.3em] mb-6 opacity-60">
            Vibe Check
          </p>
          <h3 className="text-3xl font-black uppercase italic mb-10 leading-none">
            Chạm đến...
          </h3>
          <div className="space-y-6">
            {theme.vibes.map((vibe) => (
              <div
                key={vibe}
                className="flex items-center gap-6 p-5 bg-black/20 backdrop-blur-md rounded-3xl border border-white/10 hover:bg-black/30 transition-all cursor-default group"
              >
                <div className="w-3 h-3 rounded-full bg-white animate-pulse shadow-[0_0_15px_rgba(255,255,255,0.5)]" />
                <span className="font-black text-xl tracking-tighter uppercase italic group-hover:translate-x-2 transition-transform">
                  {vibe}
                </span>
              </div>
            ))}
      ...
      <section className="py-40 border-y border-white/5 bg-slate-950 relative overflow-hidden">
        ...
        <div className="max-w-5xl mx-auto px-6 text-center relative z-10">
          <div
            className={`text-9xl mb-12 opacity-10 font-serif ${theme.colors.primary}`}
          >
            “
          </div>
          ...
          <div
            className={`w-32 h-2 bg-gradient-to-r ${theme.colors.gradient} mx-auto rounded-full`}
          />
        </div>
      </section>
      ...
      <section className="max-w-5xl mx-auto px-6 py-40 text-center">
        ...
        <button
          className={`px-20 py-8 bg-gradient-to-r ${theme.colors.gradient} rounded-[2rem] font-black uppercase tracking-[0.3em] text-lg text-white hover:scale-110 transition-all shadow-[0_40px_80px_rgba(0,0,0,0.4)]`}
        >
